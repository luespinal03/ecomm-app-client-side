import React from 'react'
import { Link } from 'react-router-dom';
import './ProductsCard.css'
import { Card, Button } from 'react-bootstrap'


const ProductsCard = ({ product, itemToShoppingCart }) => {


    return (

        <div class=" m-10 h-60vh rounded-2xl bg-gray-100 flex flex-col justify-center">
            <div class="relative m-3 flex flex-wrap mx-auto justify-center">
                <div class="relative max-w-sm min-w-[340px] bg-white shadow-md rounded-3xl p-2 mx-1 my-3 cursor-pointer">
                    <div class="overflow-x-hidden rounded-2xl relative ">
                        <img class="h-100 rounded-2xl w-full object-cover p-2" src={product.image} />
                        <button class="border-0 absolute right-2 top-2 bg-white rounded-full p-2 cursor-pointer group" onClick={() => { itemToShoppingCart(product) }}>
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 group-hover:opacity-50 opacity-70" fill="none" viewBox="0 0 24 24" stroke="black">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                        </button>
                    </div>
                    <div class="mt-4 pl-2 mb-2 flex justify-between ">
                        <div>
                            <p class="text-lg font-semibold text-gray-900 mb-0">{product.title}</p>
                            <p class="text-md text-gray-800 mt-0">${product.price}</p>

                        </div>

                        {/* wishlist icon */}
                        <div class="flex flex-col-reverse mb-1 mr-4 group cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 group-hover:opacity-70" fill="none" viewBox="0 0 24 24" stroke="gray">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                            </svg>


                        </div>
                    </div>
                </div>




                {/* <Button className='add-to-cart-button' onClick={() => { itemToShoppingCart(product) }}>Add to Cart</Button> */}

                {/* <div class="relative max-w-sm min-w-[340px] bg-white shadow-md rounded-3xl p-2 mx-1 my-3 cursor-pointer">
                    <div class="overflow-x-hidden rounded-2xl relative">
                        <img class="h-40 rounded-2xl w-full object-cover" src="https://pixahive.com/wp-content/uploads/2020/10/Gym-shoes-153180-pixahive.jpg" />
                        <p class="absolute right-2 top-2 bg-white rounded-full p-2 cursor-pointer group">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 group-hover:opacity-50 opacity-70" fill="none" viewBox="0 0 24 24" stroke="black">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                        </p>
                    </div>
                    <div class="mt-4 pl-2 mb-2 flex justify-between ">
                        <div>
                            <p class="text-lg font-semibold text-gray-900 mb-0">Product Name</p>
                            <p class="text-md text-gray-800 mt-0">$340</p>
                        </div>
                        <div class="flex flex-col-reverse mb-1 mr-4 group cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 group-hover:opacity-70" fill="none" viewBox="0 0 24 24" stroke="gray">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                            </svg>
                        </div>
                    </div>
                </div> */}

            </div>
        </div>

    )
}

export default ProductsCard







    // < div >
    // <Card style={{}} className='products-card'>
    //     <br />
    //     <Card.Body className='card-body'>
    //         <Card.Title className='product-card-title'>{product.title}</Card.Title>
    //         <br />
    //         <Card.Img variant="top" src={product.image} className='product-img' />
    //         <Card.Text className='product-price'>${product.price}</Card.Text>
    //         <Card.Text className='product-features'>{product.features}</Card.Text>
    //         <Button className='add-to-cart-button' onClick={() => { itemToShoppingCart(product) }}>Add to Cart</Button>
    //     </Card.Body>
    //     <br />
    // </Card>
    //     </ >